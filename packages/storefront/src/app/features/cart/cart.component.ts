                ...
                <div class="soft-card grid gap-5 rounded-[1.5rem] p-5 md:grid-cols-[1.1fr_0.4fr_0.4fr]">
                  <div class="flex gap-4">
                    <img
                      ...
                      [alt]="item.name"
                      class="h-24 w-24 rounded-[1.25rem] object-cover shadow-[0_18px_40px_-28px_rgba(15,23,42,0.65)]"
                    />
                    <div>
                      <div class="text-xs uppercase tracking-[0.24em] text-[#f26b38]">
                        {{ item.licenseType === 'team' ? 'Studio license' : 'Single project' }}
                      </div>
                      <div class="text-2xl font-semibold text-slate-950">{{ item.name }}</div>
                      <div class="mt-2 text-sm leading-6 text-slate-600">
                        Premium storefront UI with production-ready Tailwind markup and framework variants.
                      </div>
                      <a [routerLink]="['/product', item.slug]" class="mt-3 inline-flex text-sm font-semibold text-[#102a43]">
                        View pack
                      </a>
                      <button (click)="cart.removeItem(item.id)" class="mt-4 text-sm font-semibold text-[#f26b38]">Remove pack</button>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div class="mt-3 flex items-center gap-2">
                      <button (click)="item.quantity > 1 ? cart.updateQuantity(item.id, item.quantity - 1) : cart.removeItem(item.id)" class="inline-flex h-10 w-10 items-center justify-center rounded-full border border-slate-200 bg-white text-slate-700">-</button>
